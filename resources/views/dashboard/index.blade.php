...
.summary-copy{font-size:15px;line-height:1.78;color:#354238;white-space:pre-wrap;text-align:justify;text-align-last:left;text-justify:inter-word;hyphens:auto}
/* Interactive Legal Glossary: display-only highlights for generated summaries. */
.legal-glossary-term{position:relative;display:inline;border-radius:4px;background:#fff3bf;border-bottom:1px dotted #8a6b00;color:inherit;cursor:help;padding:0 2px}
.legal-glossary-term:focus{outline:2px solid rgba(201,160,0,.35);outline-offset:2px}
.legal-glossary-term::after{content:attr(data-definition);position:absolute;left:50%;bottom:calc(100% + 8px);z-index:40;width:max-content;max-width:min(280px,80vw);transform:translate(-50%,4px);padding:9px 11px;border-radius:10px;background:#173d2b;color:#fff;font-size:12px;font-weight:600;line-height:1.45;white-space:normal;box-shadow:0 10px 28px rgba(0,0,0,.18);opacity:0;visibility:hidden;pointer-events:none;transition:opacity .16s ease,transform .16s ease,visibility .16s ease}
.legal-glossary-term::before{content:'';position:absolute;left:50%;bottom:calc(100% + 3px);z-index:41;transform:translateX(-50%);border:5px solid transparent;border-top-color:#173d2b;opacity:0;visibility:hidden;transition:opacity .16s ease,visibility .16s ease}
.legal-glossary-term:hover::after,.legal-glossary-term:focus::after,.legal-glossary-term.glossary-open::after{opacity:1;visibility:visible;transform:translate(-50%,0)}
.legal-glossary-term:hover::before,.legal-glossary-term:focus::before,.legal-glossary-term.glossary-open::before{opacity:1;visibility:visible}
...
.dictionary-toolbar{display:flex;align-items:center;justify-content:space-between;gap:14px;flex-wrap:wrap;margin-bottom:14px}
.dictionary-search{max-width:420px;flex:1 1 260px}
.dictionary-table-wrap{overflow:auto;border:1px solid rgba(26,71,49,.08);border-radius:14px;background:#fff}
.dictionary-table{width:100%;min-width:620px;border-collapse:collapse}
.dictionary-table th{padding:13px 16px;text-align:left;background:#f7faf7;color:#2d6a4f;font-size:11px;letter-spacing:.08em;text-transform:uppercase;border-bottom:1px solid rgba(26,71,49,.08)}
.dictionary-table td{padding:15px 16px;border-bottom:1px solid rgba(26,71,49,.07);vertical-align:top;color:#39463d;line-height:1.55}
.dictionary-table tr:last-child td{border-bottom:none}
.dictionary-term{font-weight:900;color:#1a4731;text-transform:capitalize}
.dictionary-meaning{font-size:14px}
...
.dataset-toolbar{display:flex;align-items:center;justify-content:space-between;gap:14px;flex-wrap:wrap;margin-bottom:14px}
.dataset-table-wrap{overflow:auto;border:1px solid rgba(26,71,49,.08);border-radius:14px;background:#fff}
.dataset-table{min-width:1100px;margin:0}
.dataset-text-cell{max-width:360px;white-space:normal;line-height:1.5;color:#38443d;font-size:13px}
.dataset-output-cell{max-width:420px}
.dataset-meta{display:flex;gap:8px;flex-wrap:wrap;margin-top:8px}
.dataset-meta .law-chip{padding:5px 8px;font-size:11px}
...
@media (max-width: 768px){.summary-two-col{grid-template-columns:1fr}.summary-complete-banner{align-items:flex-start;flex-direction:column}.admin-status-bar{grid-template-columns:repeat(2,minmax(0,1fr))}.legal-glossary-term::after{left:0;max-width:min(260px,calc(100vw - 48px));transform:translate(0,4px)}.legal-glossary-term:hover::after,.legal-glossary-term:focus::after,.legal-glossary-term.glossary-open::after{transform:translate(0,0)}.legal-glossary-term::before{left:12px;transform:none}}
</style>
</head>
<body class="app-body">
...
<aside class="app-sidebar" id="appSidebar">
  ...
  <nav class="sb-nav">
    ...
    @if($isAdmin)
      ...
      <div class="sb-item" id="nav-dataset" onclick="goTab('dataset')">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M4 6c0-1.1 3.58-2 8-2s8 .9 8 2-3.58 2-8 2-8-.9-8-2z"/><path d="M4 6v6c0 1.1 3.58 2 8 2s8-.9 8-2V6"/><path d="M4 12v6c0 1.1 3.58 2 8 2s8-.9 8-2v-6"/></svg>Dataset
      </div>
    @endif
    ...
    @else
      <div class="sb-section-lbl">Learning</div>
      <div class="sb-item" id="nav-dictionary" onclick="goTab('dictionary')">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M4 4.5A2.5 2.5 0 0 1 6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5z"/></svg>Dictionary
      </div>
    @endif
...
<div class="app-main" id="appMain">
  ...
  <div id="tab-new-job" class="hidden">
    <div class="topbar"><div><div class="topbar-title">New Job</div><div class="topbar-sub">Upload a PDF legal document to generate a summary using Natural Language Processing and a fine-tuned BART model trained on ZASCA and Zimbabwean data</div></div></div>
    <div class="content">
      <div class="job-grid">
        <div>
          <div class="upload-zone" id="uploadZone" onclick="document.getElementById('fileInput').click()">
            <input type="file" id="fileInput" accept=".pdf,application/pdf" style="display:none" onchange="onFileChosen(event)">
            ...
            <div class="upload-sub">Drag &amp; drop or click to browse a PDF</div>
      ...
      <!-- Progress bar -->
      <div class="progress-wrap progress-loader-card hidden" id="progressWrap" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0">
        <div class="progress-ring progress-loader-circle" id="progressRing" aria-hidden="true">
          <div class="progress-ring-inner progress-loader-core"><span id="progressPercent">0%</span></div>
        </div>
  ...
  <!-- ─── TAB: PROFILE ─── -->
  @unless($isLegalProfessional)
  <div id="tab-dictionary" class="hidden">
    <div class="topbar">
      <div>
        <div class="topbar-title">Dictionary</div>
        <div class="topbar-sub">Simple meanings for hard court words used in summaries</div>
      </div>
    </div>
    <div class="content">
      <div class="panel">
        <div class="panel-head">
          <div>
            <div class="panel-title">Court Words And Meanings</div>
            <div class="panel-sub" id="dictionaryCount">Loading terms...</div>
          </div>
        </div>
        <div class="insight-panel-body">
          <div class="dictionary-toolbar">
            <div class="search-wrap dictionary-search">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              <input class="search-inp" id="dictionarySearch" placeholder="Search a court word..." oninput="renderDictionary()">
            </div>
            <span class="law-chip blue">Plain English</span>
          </div>
  ...
  <div id="tab-dataset" class="hidden">
    <div class="topbar">
      <div>
        <div class="topbar-title">Dataset</div>
        <div class="topbar-sub">Input-output examples from processed legal documents</div>
      </div>
      <button class="btn btn-yellow" onclick="exportDatasetCsv()">
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>Export CSV
      </button>
    </div>
    <div class="content">
      <div class="panel">
        <div class="panel-head">
          <div>
            <div class="panel-title">Input-Output Dataset</div>
            <div class="panel-sub" id="datasetCount">Loading examples...</div>
          </div>
          <span class="admin-badge">Admin Dataset</span>
        </div>
        <div class="insight-panel-body">
          <div class="dataset-toolbar">
            <div class="mini-note">The CSV export includes full input text and full target output.</div>
            <button class="btn btn-ghost btn-sm" onclick="loadDataset(true)">Refresh</button>
          </div>
          <div id="datasetTable"><div class="loading-state">Loading dataset...</div></div>
        </div>
      </div>
    </div>
  </div>
        ...
        <div class="insight-panel soft">
          ...
          <div class="insight-panel-body">
            <div class="passage-list">
              ...
              <div class="passage-text">Open <strong>New Job</strong> and upload a PDF legal document.</div>
              </div>
              <div class="passage-card">
...
const ENDPOINTS = {
  ...
  adminDataset: @json(route('admin.dataset')),
  adminDatasetExport: @json(route('admin.dataset.export')),
...
// Interactive Legal Glossary: loaded from a Blade component so the dictionary is reusable and display-only.
const LEGAL_GLOSSARY = loadLegalGlossaryTerms();
const LEGAL_GLOSSARY_PATTERN = buildLegalGlossaryPattern(LEGAL_GLOSSARY);
...
const S = {
  ...
  currentSummary: null, records: [], downloads: [], dataset: [],
};
...
// ══ TAB NAVIGATION ═════════════════════════
const TABS = ['dashboard', 'new-job', 'records', 'downloads', ...(!IS_LEGAL_PROFESSIONAL ? ['dictionary'] : []), ...(IS_ADMIN ? ['dataset'] : []), ...(IS_LEGAL_PROFESSIONAL ? ['legal-search', 'clause-extractor'] : []), 'help', 'settings'];
...
function onFileChosen(e) {
  ...
  document.getElementById('fileMeta').textContent = fmtSize(file.size) + ' · PDF document';
...
function updateProgressUi(progress) {
  ...
  document.getElementById('progressPercent').textContent = roundedValue + '%';
  document.getElementById('progressRing').style.background = `conic-gradient(var(--green-bright) ${value}%, var(--grey-100) 0)`;
  progressWrap.setAttribute('aria-valuenow', String(roundedValue));
  progressWrap.setAttribute('aria-label', `${roundedValue}% - ${progress?.step || 'Working'}`);
...
function modeSummaryHtml(summaryType, sections, fallback) {
  const documentType = String(fallback.s.document_type || '');
  const isLegislativeDocument = /^(Act|Bill|Statutory Instrument)$/i.test(documentType);

  if (isLegislativeDocument) {
    const legislativeSections = Object.keys(sections || {}).length ? sections : {
      'Summary': bestSummaryText(fallback.citizenSummary, fallback.executiveSummary, fallback.professionalSummary),
      'Document Type': documentType || 'Legislative Instrument',
      'Legislative Subject': bestSectionText(fallback.s.key_findings, fallback.citizenSummary, 45),
      'Legal Effect': bestSectionText(fallback.s.outcome, '', 8),
      'What This Means': bestSectionText(fallback.s.practical_implications, '', 15),
    };
    const title = summaryType === 'legal_professional' ? 'Legislative Analysis' : 'Legislative Summary';
    const sub = summaryType === 'legal_professional'
      ? 'Formal analysis of rules, powers, duties, amendments, and compliance consequences.'
      : 'Plain-language explanation of the law, who it affects, and what it requires.';

    return `<div class="summary-simple-card">
      <div class="summary-simple-head">
        <div>
          <div class="summary-simple-title">${title}</div>
          <div class="summary-simple-sub">${sub}</div>
        </div>
      </div>
      <div class="summary-mode-grid">${sectionCards(legislativeSections)}</div>
    </div>`;
  }

  if (summaryType === 'legal_professional') {
    const legalSections = {
      'Summary': bestSummaryText(sections.Summary, fallback.professionalSummary, fallback.executiveSummary),
      ...
      'Facts': bestSectionText(sections.Facts, fallback.s.key_findings || fallback.professionalSummary, 45),
      'Legal Issues': sections['Legal Issues'] || fallback.panels.key_legal_issues || [],
      'Holding / Decision': bestSectionText(sections['Holding / Decision'], fallback.s.outcome, 8),
      'Ratio Decidendi': bestSectionText(sections['Ratio Decidendi'], fallback.s.legal_principles, 12),
  ...
  const generalSections = {
    'Summary': bestSummaryText(sections.Summary, fallback.citizenSummary, fallback.executiveSummary),
    'Document Overview': bestSectionText(sections['Document Overview'] || sections['What Happened'], fallback.s.key_findings || fallback.citizenSummary, 45),
    'Main Issue': sections['Main Issue'] || fallback.panels.key_legal_issues || fallback.findings,
    'Decision / Outcome': bestSectionText(sections['Decision / Outcome'], fallback.s.outcome, 8),
    'What This Means': bestSectionText(sections['What This Means'], fallback.s.practical_implications, 15),
  };
...
function bestSummaryText(...values) {
...
function bestSectionText(savedValue, fallbackValue, minimumSavedWords = 20) {
  const saved = String(savedValue || '').trim();
  const fallback = String(fallbackValue || '').trim();
  if (!saved) return fallback;
  if (!fallback) return saved;
  return wordCount(saved) < minimumSavedWords && wordCount(fallback) > wordCount(saved) ? fallback : saved;
}
...
function loadLegalGlossaryTerms() {
  const element = document.getElementById('legalGlossaryTerms');
  if (!element) return {};

  try {
    const parsed = JSON.parse(element.textContent || '{}');
    return Object.fromEntries(Object.entries(parsed).map(([term, definition]) => [
...
function buildLegalGlossaryPattern(glossary) {
...
function renderDictionary() {
  const rows = document.getElementById('dictionaryRows');
  if (!rows) return;

  const query = String(document.getElementById('dictionarySearch')?.value || '').trim().toLowerCase();
  const terms = Object.entries(LEGAL_GLOSSARY || {})
    .map(([term, meaning]) => ({ term, meaning }))
    .filter(item => !query || item.term.toLowerCase().includes(query) || String(item.meaning).toLowerCase().includes(query))
    .sort((a, b) => a.term.localeCompare(b.term));

  const count = document.getElementById('dictionaryCount');
  if (count) count.textContent = `${terms.length} term${terms.length === 1 ? '' : 's'} available`;

  rows.innerHTML = terms.length
    ? terms.map(item => `<tr><td class="dictionary-term">${esc(item.term)}</td><td class="dictionary-meaning">${esc(item.meaning)}</td></tr>`).join('')
    : '<tr><td colspan="2" class="dictionary-meaning">No matching terms found.</td></tr>';
}
...
// Wrap legal terms in escaped summary text without changing the raw summary/export data.
function legalGlossaryHtml(text) {
  const source = String(text ?? '');
  if (!source || !LEGAL_GLOSSARY_PATTERN) return esc(source);

  let html = '';
  let lastIndex = 0;
  LEGAL_GLOSSARY_PATTERN.lastIndex = 0;

  source.replace(LEGAL_GLOSSARY_PATTERN, (match, term, offset) => {
    const definition = LEGAL_GLOSSARY[String(term).toLowerCase()];
    if (!definition) return match;

    html += esc(source.slice(lastIndex, offset));
    html += `<span class="legal-glossary-term" tabindex="0" role="button" aria-label="${escAttr(match + ': ' + definition)}" data-definition="${escAttr(definition)}">${esc(match)}</span>`;
...
function sectionCards(sections) {
  return Object.entries(sections).map(([title, value]) => {
    const wordLimit = title === 'Summary' ? 300 : (['Document Overview', 'What Happened'].includes(title) ? 150 : (title === 'Facts' ? 130 : 105));
    const sentenceLimit = title === 'Summary' ? 12 : 2;
    const listWordLimit = title === 'Main Issue' ? 42 : (title === 'Legal Issues' ? 36 : 24);
    const body = Array.isArray(value)
      ? simpleList(compactListItems(value, 5, listWordLimit), 'Not clearly stated.', true)
      : `<div class="summary-copy">${legalGlossaryHtml(compactDisplayText(value || 'Not clearly stated.', wordLimit, sentenceLimit))}</div>`;
...
function compactListItems(items, itemLimit = 5, wordLimit = 18) {
  return (Array.isArray(items) ? items : [])
    .map(item => {
      const value = item && typeof item === 'object' ? (item.text || item.title || item.value || '') : item;
      return compactDisplayText(value, wordLimit);
    })
    .filter(Boolean)
    .slice(0, itemLimit);
}

function simpleList(items, emptyText, highlightTerms = false) {
  const list = Array.isArray(items) ? items.filter(Boolean) : [];
  if (!list.length) return `<div class="mini-note" style="margin-top:12px">${esc(emptyText)}</div>`;
  return `<ul class="summary-list" style="margin-top:12px">${list.map(item => `<li>${highlightTerms ? legalGlossaryHtml(item) : esc(item)}</li>`).join('')}</ul>`;
}
...
function renderClauses(clauses) {
  ...
  return clauses.slice(0, 10).map(clause => `<div class="passage-card">
    <div class="passage-meta">${esc(clause.clause_type || 'GENERAL_CLAUSE')}</div>
    <div class="trace-claim">${esc(compactDisplayText(clause.heading || 'Clause', 16))}</div>
    <div class="passage-text">${esc(compactDisplayText(clause.content || '', 70))}</div>
  </div>`).join('');
}
...
// Mobile-friendly glossary tap support; hover/focus still works through CSS.
document.addEventListener('click', event => {
  const term = event.target.closest?.('.legal-glossary-term');
  document.querySelectorAll('.legal-glossary-term.glossary-open').forEach(openTerm => {
    if (openTerm !== term) openTerm.classList.remove('glossary-open');
  });

  if (term) {
    term.classList.toggle('glossary-open');
  }
});

document.addEventListener('keydown', event => {
  if (event.key === 'Escape') {
    document.querySelectorAll('.legal-glossary-term.glossary-open').forEach(term => term.classList.remove('glossary-open'));
  }
});
...
function detectClauses(text) {
  const lines = text.replace(/\r/g, '').split('\n').map(line => line.trim()).filter(line => line && !isClauseNoiseLine(line));
  const clauses = [];
  let current = null;

  for (const line of lines) {
    if (looksLikeClauseHeading(line)) {
      pushDetectedClause(clauses, current);
      current = { clause_type: classifyClause(line), heading: cleanClauseHeading(line), content: '' };
      continue;
    }

    if (current && !isClauseNoiseLine(line)) {
...
function pushDetectedClause(clauses, clause) {
  if (!clause || !clause.content) return;
  const content = compactDisplayText(clause.content, 70);
  if (content.length < 20 || isClauseNoiseLine(content)) return;
  clauses.push({
    clause_type: clause.clause_type || classifyClause(clause.heading || ''),
    heading: compactDisplayText(cleanClauseHeading(clause.heading || 'Clause'), 16),
    content,
  });
}

function looksLikeClauseHeading(line) {
  const headingPattern = /^((section|clause|article|part|chapter|schedule)\s+)?\d+[A-Z]?(\.\d+)*\s*[-.)]?\s+.{3,140}$|^(definitions?|interpretation|orders?|remedies|obligations?|termination|confidentiality|liability|indemnity|jurisdiction|governing law|payment|salary|duties|powers|offences|penalties|warranties|notices?|dispute resolution)\b.*$/i;
  const uppercaseClauseHeading = /^[A-Z][A-Z0-9\s,.'&()/:-]{8,120}$/.test(line)
    && /\b(DEFINITIONS?|INTERPRETATION|ORDERS?|REMEDIES|OBLIGATIONS?|TERMINATION|CONFIDENTIALITY|LIABILITY|INDEMNITY|JURISDICTION|GOVERNING LAW|PAYMENT|SALARY|DUTIES|POWERS|OFFENCES|PENALTIES|WARRANTIES|NOTICES?|DISPUTE RESOLUTION)\b/.test(line);
  return headingPattern.test(line) || uppercaseClauseHeading;
}

function cleanClauseHeading(line) {
  return String(line || '')
    .replace(/^(?:-{3}|[\u2500\u2501]{3})\s*PAGE\s+\d+\s*(?:-{3}|[\u2500\u2501]{3})?/i, '')
    .replace(/\s+/g, ' ')
    .trim();
}

function compactDisplayText(text, wordLimit = 70, sentenceLimit = 2) {
  const clean = String(text || '')
    .replace(/(?:-{3}|[\u2500\u2501]{3})\s*PAGE\s+\d+\s*(?:-{3}|[\u2500\u2501]{3})?/ig, ' ')
    .replace(/\bPage\s+\d+\s+(?:of|\/)\s+\d+\b/ig, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  if (!clean) return '';

  const selected = (clean.match(/[^.!?]+[.!?]?/g) || [clean])
    .map(sentence => sentence.trim())
    .filter(sentence => sentence.length >= 20 && !isClauseNoiseLine(sentence))
    .slice(0, sentenceLimit)
    .join(' ') || clean;

  const words = selected.split(/\s+/).filter(Boolean);
  return words.length > wordLimit ? words.slice(0, wordLimit).join(' ') + '...' : selected;
}

function isClauseNoiseLine(line) {
  const value = String(line || '').trim();
  if (!value) return true;
  return /^(?:page\s+\d+(?:\s+(?:of|\/)\s+\d+)?|\d+|harare|bulawayo|mutare|in the .*court|before:|between|and|-{3}\s*page\s+\d+\s*-{3})$/i.test(value)
    || /^(?:applicant|respondent|plaintiff|defendant|before|coram)\s*:?$/i.test(value);
}
...
function catBadge(type) {
  ...
  if (/act|bill|regulation|statute|statutory/.test(t)) return 'act';
  return 'other';
}
...
async function loadDataset(force = false) {
  ...
  if (S.dataset.length && !force) {
    renderDataset(S.dataset, S.datasetTotal || S.dataset.length);
    return;
  }

  tableEl.innerHTML = '<div class="loading-state">Loading dataset...</div>';
  countEl.textContent = 'Loading examples...';

  try {
    const res = await fetch(ENDPOINTS.adminDataset + '?limit=100', { headers: { Accept: 'application/json' } });
    if (!res.ok) throw new Error('dataset request failed');
    const data = await res.json();
    S.dataset = data.rows || [];
    S.datasetTotal = data.total || S.dataset.length;
    renderDataset(S.dataset, S.datasetTotal);
  } catch {
    tableEl.innerHTML = '<div class="empty-state">Could not load dataset rows.</div>';
    countEl.textContent = 'Dataset unavailable';
  }
}

function renderDataset(list, total) {
  const tableEl = document.getElementById('datasetTable');
  const countEl = document.getElementById('datasetCount');
  countEl.textContent = `${total || list.length} dataset row${(total || list.length) === 1 ? '' : 's'}`;

  if (!list.length) {
    tableEl.innerHTML = '<div class="empty-state"><div class="empty-icon">Data</div><div class="empty-title">No dataset rows yet</div><div class="empty-sub">Completed summaries with extracted source text will appear here.</div></div>';
    return;
  }

  tableEl.innerHTML = `<div class="dataset-table-wrap"><table class="rec-table dataset-table">
    <thead><tr><th>Document</th><th>Type</th><th>Input</th><th>Output</th><th>Mode</th><th>Created</th></tr></thead>
    <tbody>
      ${list.map(row => `<tr>
        <td>
          <strong>${esc(row.document_name || 'Document')}</strong>
          <div class="dataset-meta">
            <span class="law-chip">Doc #${esc(row.document_id || '')}</span>
            <span class="law-chip blue">Summary #${esc(row.summary_id || '')}</span>
          </div>
        </td>
        <td><span class="type-badge ${catBadge(row.document_type)}">${esc(row.document_type || 'Legal Document')}</span></td>
        <td class="dataset-text-cell">${esc(row.input_preview || '')}</td>
        <td class="dataset-text-cell dataset-output-cell">${esc(row.output_preview || '')}</td>
        <td>${esc((row.summary_type || 'general_user').replace(/_/g, ' '))}</td>
...
function exportDatasetCsv() {
  if (!IS_ADMIN) return;
  window.location.href = ENDPOINTS.adminDatasetExport;
}
...
function generatePDF(s, filename) {
  ...
  function drawJustifiedLine(line, isLastLine) {
    ...
    const extraSpace = (CW - naturalWidth) / (words.length - 1);
    if (extraSpace <= 0 || extraSpace > 3) {
      doc.text(line, M, y);
      return;
    }
    ...
    words.forEach((word, index) => {
      doc.text(word, x, y);
      x += doc.getTextWidth(word) + baseSpace + (index < words.length - 1 ? extraSpace : 0);
    });
  }
  function bodyTxt(t) {
    if(!t) return;
    doc.setFont('helvetica','normal'); doc.setFontSize(10); doc.setTextColor(60,60,55);
    String(t).split(/\n{2,}/).map(p => p.trim()).filter(Boolean).forEach(paragraph => {
      const lines = doc.splitTextToSize(paragraph, CW);
      lines.forEach((line, index) => {
        newPage();
        drawJustifiedLine(line, index === lines.length - 1);
        y += 5.5;
      });
      y += 4;
    });
