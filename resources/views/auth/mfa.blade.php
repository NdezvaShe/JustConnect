...
.mfa-status{padding:12px 14px;border-radius:12px;background:#ecfdf3;color:#1a4731;margin-bottom:16px;font-weight:700}
...
<body>
  <form class="mfa-card" method="POST" action="{{ route('mfa.verify') }}">
    ...
    @endif
    @if(session('status'))
      <div class="mfa-status">{{ session('status') }}</div>
    @endif
    ...
    <div class="mfa-actions">
      ...
      <button class="btn btn-outline" type="submit" formaction="{{ route('mfa.resend') }}" formmethod="POST">Resend code</button>
