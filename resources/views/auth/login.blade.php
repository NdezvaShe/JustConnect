    ...
    <div class="auth-box">
      ...
      <form method="POST" action="{{ route('login') }}">
        ...
        <div class="form-group">
          ...
          <input type="password" name="password" id="loginPassword" class="form-input @error('password') err @enderror" placeholder="Your password" required>
          @error('password')<div class="form-err show">{{ $message }}</div>@enderror
        </div>
        <div class="form-check-row">
          <label class="form-check"><input type="checkbox" id="showPasswordToggle"> Show password</label>
          <a href="{{ route('password.forgot') }}" style="color:#2d6a4f;text-decoration:none;font-weight:800">Forgot password?</a>
        </div>
