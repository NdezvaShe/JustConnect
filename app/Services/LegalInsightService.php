...
class LegalInsightService
{
    ...
    public function buildAiContext(string $text, array $nlpResult): array
    {
        ...
        return [
            'passages' => $passages,
            'prompt_text' => $this->formatAiPassages($passages, $nlpResult, $text),
        ];
    }
    ...
    public function enrich(string $text, array $result, array $context = []): array
    {
        ...
        if ($this->isLegislativeDocumentType((string) ($result['document_type'] ?? ''))) {
            $result['executive_summary'] = ($result['summary_type'] ?? null) === SummaryPromptTemplateService::GENERAL_USER
                ? $citizenSummary
                : $professionalSummary;
        }
    ...
    private function formatAiPassages(array $passages, array $nlpResult, string $sourceText = ''): string
    {
        $header = [
            ...
            'Candidate parties from local NLP, verify against the caption before using: ' . implode(' | ', array_slice($nlpResult['parties'] ?? [], 0, 6)),
            'Keywords: ' . implode(', ', array_slice($nlpResult['nlp_keywords'] ?? [], 0, 8)),
        ];

        if ($this->isLegislativeDocumentType((string) ($nlpResult['document_type'] ?? ''))) {
            $header[] = 'Opening keyword hint: ' . $this->firstMeaningfulWords($sourceText, 20);
            $header[] = 'Instruction: this is legislation. Treat it as rules, powers, duties, amendments, prohibitions, penalties, and compliance effects. Do not narrate it as a dispute, event, facts, court reasoning, or final ruling.';
        } else {
            $header[] = 'Instruction: use the opening excerpt for caption/parties and the ending excerpt for the final order. Ignore cited authorities when identifying parties.';
        }

        $sourceText = $this->normaliseParagraph($sourceText);
        $opening = $sourceText !== '' ? mb_substr($sourceText, 0, 1800) : '';
        $ending = mb_strlen($sourceText) > 1800 ? mb_substr($sourceText, -1800) : '';
        $contextExcerpts = array_filter([
            $opening !== '' ? '[Opening excerpt for parties/caption] ' . $opening : null,
            $ending !== '' ? '[Ending excerpt for outcome/order] ' . $ending : null,
        ]);
        ...
        return implode("\n\n", array_filter(array_merge($header, [''], $contextExcerpts, [''], $items)));
    }

    private function professionalSummary(array $result, array $supportingPassages): string
    {
        if ($this->isLegislativeDocumentType((string) ($result['document_type'] ?? ''))) {
            return $this->legislativeSummary($result, $supportingPassages, true);
        }

        $provided = $this->normaliseParagraph((string) ($result['professional_summary'] ?? ''));
        if ($provided !== '' && !$this->shouldRegenerateProfessionalSummary($provided, $result)) {
            return $this->shortSnippet($provided, 300);
        }
        ...
        $clauses = $this->clauseHighlights($result['structured_panels']['key_clauses'] ?? ($result['clauses'] ?? []), 4);
        $support = $this->listToSentence($this->passageHighlights($supportingPassages, 2, 120));

        $parts = array_filter([
            $intro,
            $issues ? 'The central legal issues include ' . $issues . '.' : null,
            $clauses ? 'Material clauses or operative provisions include ' . $this->listToSentence($clauses) . '.' : null,
        ...
        return $this->shortSnippet($this->normaliseParagraph(implode(' ', $parts)), 300);
    }

    private function shouldRegenerateProfessionalSummary(string $summary, array $result): bool
    {
        ...
        $docType = (string) ($result['document_type'] ?? '');

        if (preg_match('/\b(this document explains|in everyday terms|what this means|simple terms|people affected by it)\b/u', $lower)) {
            return true;
        }

        if ($docType === 'Court Judgment' && !preg_match('/\b(court|held|found|ordered|dismissed|granted|ratio|issue|applicant|respondent|appeal|application)\b/u', $lower)) {
            return true;
        }

        if (!$this->summaryMentionsAny($summary, array_merge(
    ...
    private function summaryMentionsAny(string $summary, array $values): bool
    {
        $lower = mb_strtolower($summary);
        foreach ($values as $value) {
            ...
            foreach ($tokens as $token) {
                if (mb_strlen($token) >= 5 && str_contains($lower, $token)) {
                    return true;
                }
            }
        }
    ...
    private function clauseHighlights(array $clauses, int $limit): array
    {
        $items = [];
        foreach ($clauses as $clause) {
            $heading = trim((string) ($clause['heading'] ?? ''));
            $content = $this->normaliseParagraph((string) ($clause['content'] ?? ''));
            if ($content === '') {
                continue;
            }

            $items[] = ($heading !== '' ? $heading . ': ' : '') . $this->shortSnippet($content, 28);
    ...
    {
        if ($this->isLegislativeDocumentType((string) ($result['document_type'] ?? ''))) {
            return $this->legislativeSummary($result, $supportingPassages, false);
        }
    ...
    private function legislativeSummary(array $result, array $supportingPassages, bool $formal): string
    {
        $intro = $this->buildIntro($result, $formal);
        $findings = $this->normaliseParagraph((string) ($result['key_findings'] ?? ''));
        $obligations = $this->listToSentence($result['key_obligations'] ?? []);
        $effect = $this->normaliseParagraph((string) ($result['outcome'] ?? ''));
        $implications = $this->normaliseParagraph((string) ($result['practical_implications'] ?? ''));
        $references = $this->listToSentence($result['legal_references'] ?? []);
        $highlights = $this->listToSentence($this->legislativePassageHighlights($supportingPassages, 3, 42));

        if ($formal) {
            $parts = array_filter([
                $intro,
                $findings !== '' ? 'The principal provisions indicate that ' . $this->lowercaseFirst($findings) : null,
                $obligations !== '' ? 'Operative duties and powers include ' . $obligations . '.' : null,
                $effect !== '' ? 'The legal effect is that ' . $this->lowercaseFirst($effect) : null,
                $implications !== '' ? 'Compliance implications include that ' . $this->lowercaseFirst($implications) : null,
                $references !== '' ? 'Relevant references include ' . $references . '.' : null,
                $highlights !== '' ? 'The extracted provisions also show that ' . $highlights . '.' : null,
            ]);

            return $this->cleanLegislativeSummary($this->normaliseParagraph(implode(' ', $parts)));
        }

        $parts = array_filter([
            $intro,
            $findings !== '' ? 'In simple terms, it deals with ' . $this->lowercaseFirst($findings) : null,
            $obligations !== '' ? 'It requires or allows the following: ' . $obligations . '.' : null,
            $effect !== '' ? 'Its legal effect is that ' . $this->lowercaseFirst($effect) : null,
            $implications !== '' ? 'People affected by it should check what they must do, what approvals or records are needed, and what penalties may apply.' : null,
            $highlights !== '' ? 'The extracted provisions mention ' . $highlights . '.' : null,
        ]);

        return $this->cleanLegislativeSummary($this->normaliseParagraph(implode(' ', $parts)));
    }

    private function legislativePassageHighlights(array $supportingPassages, int $limit, int $wordLimit): array
    {
        $items = [];
        foreach ($supportingPassages as $passage) {
            $text = $this->trimSentence((string) ($passage['text'] ?? ''));
            if ($text === '' || $this->containsCaseNarration($text)) {
                continue;
            }

            if (!preg_match('/\b(shall|must|may|register|registration|officer|minister|penalty|offence|certificate|customary|marriage|guardian|consent|record|extract|regulations?|amend(?:ed|ment)?|repeal|commencement)\b/iu', $text)) {
                continue;
            }

            $items[] = $this->shortSnippet($this->normaliseParagraph($text), $wordLimit);
            if (count($items) >= $limit) {
                break;
            }
        }
    ...
    private function cleanLegislativeSummary(string $summary): string
    {
        $summary = preg_replace('/\bThe main facts are that\b/iu', 'The Act provides that', $summary);
        $summary = preg_replace('/\bAt its core, the document is about\b/iu', 'The legal area covered is', (string) $summary);
        $summary = preg_replace('/\bThe court reasoned that\b/iu', 'The Act provides that', (string) $summary);
        $summary = preg_replace('/\bIn everyday terms, the result is that\b/iu', 'The legal effect is that', (string) $summary);
        $summary = preg_replace('/\bwhat happened\b/iu', 'what the law requires', (string) $summary);
        $summary = preg_replace('/\bdispute\b/iu', 'legal issue', (string) $summary);

        return $this->normaliseParagraph((string) $summary);
    }

    private function containsCaseNarration(string $text): bool
    {
        return preg_match('/\b(applicant|respondent|plaintiff|defendant|accused|deceased|court reasoned|court found|appeal|application dismissed|application granted|with costs|testified|witness)\b/iu', $text) === 1;
    }
    ...
    private function buildIntro(array $result, bool $formal): string
    {
        ...
        if ($this->isLegislativeDocumentType($docType)) {
            $subject = trim((string) ($result['document_type'] ?? 'legislative document'));
            $effect = $this->normaliseParagraph((string) (($result['outcome'] ?? '') ?: ($result['practical_implications'] ?? '')));

            if ($formal) {
                return 'This ' . mb_strtolower($subject) . ' sets out legislative rules, powers, duties, and compliance consequences'
                    . ($date ? ' in text dated ' . $date : '')
                    . ($effect !== '' ? ', including that ' . $this->lowercaseFirst($this->shortSnippet($effect, 28)) : '')
                    . '.';
            }

            return 'This ' . mb_strtolower($subject) . ' explains legal rules people or institutions may need to follow'
                . ($effect !== '' ? ', including that ' . $this->lowercaseFirst($this->shortSnippet($effect, 24)) : '')
                . '.';
        }
    ...
    private function firstMeaningfulWords(string $text, int $limit): string
    {
        $words = preg_split('/\s+/u', trim((string) preg_replace('/[^\p{L}\p{N}\s.-]+/u', ' ', mb_substr($text, 0, 1400))), -1, PREG_SPLIT_NO_EMPTY) ?: [];
        $stop = array_flip(['the', 'and', 'or', 'of', 'to', 'in', 'a', 'an', 'for', 'by', 'with', 'as', 'is', 'are']);
        $words = array_values(array_filter($words, fn ($word) => !isset($stop[mb_strtolower($word)])));
    ...
    private function resultCards(string $text, array $result, array $structuredPanels, array $supportingPassages): array
    {
        $cards = [];
        if ($this->isLegislativeDocumentType((string) ($result['document_type'] ?? ''))) {
            return $this->legislativeResultCards($result, $structuredPanels);
        }
    ...
    private function structuredPanels(string $text, array $result, array $labelledEntities, array $supportingPassages): array
    {
        ...
        $isLegislative = $this->isLegislativeDocumentType((string) ($result['document_type'] ?? ''));
    ...
    private function legislativeResultCards(array $result, array $structuredPanels): array
    {
        $cards = [];

        $obligations = array_values(array_filter((array) ($result['key_obligations'] ?? [])));
        if ($obligations !== []) {
            $cards[] = [
                'title' => 'Operative Provisions',
                'tone' => 'info',
                'items' => array_slice($obligations, 0, 5),
            ];
        }

        $references = array_values(array_filter($structuredPanels['important_legal_references'] ?? []));
        if ($references !== []) {
            $cards[] = [
                'title' => 'Legal References',
                'tone' => 'neutral',
                'items' => array_slice($references, 0, 5),
            ];
        }
    ...
    private function normaliseClauses(array $clauses): array
    {
        ...
        foreach ($clauses as $clause) {
            ...
            $content = $this->cleanClauseContent((string) ($clause['content'] ?? ''));
    ...
    private function cleanClauseContent(string $content): string
    {
        ...
        $content = $this->normaliseParagraph((string) $content);
        if ($content === '') {
            return '';
        }
        ...
        foreach ($sentences as $sentence) {
            $sentence = $this->normaliseParagraph($sentence);
            if ($sentence === '' || mb_strlen($sentence) < 20 || $this->isNoisyClauseSentence($sentence)) {
                continue;
            }
        ...
        return $this->shortSnippet($content, 70);
    }

    private function isNoisyClauseSentence(string $sentence): bool
    {
        return preg_match('/^(?:page\s+\d+|harare|bulawayo|mutare|in the .*court|before:|between)\b/iu', trim($sentence)) === 1;
    }
    ...
    private function issues(string $text, array $categories, array $keywords, bool $isLegislative = false): array
    {
        if ($isLegislative) {
            return $this->legislativeIssues($text, $keywords);
        }
    ...
    private function legislativeIssues(string $text, array $keywords): array
    {
        $haystack = mb_strtolower($text . ' ' . implode(' ', $keywords));
        $issues = [];

        foreach ([
            'Customary marriage registration' => '/customary|marriage|solemn/i',
            'Marriage officers and registers' => '/officer|register|extract|certificate|custody/i',
            'Consent and guardianship requirements' => '/guardian|consent|woman|spouse|parties/i',
            'Statutory duties and compliance' => '/shall|must|required|compliance|duty|obligation/i',
            'Offences and penalties' => '/offence|penalty|liable|guilty|fine|imprison/i',
            'Ministerial or regulatory powers' => '/minister|regulation|prescribe|appoint/i',
            'Amendment or repeal effect' => '/amend|repeal|substitut|insert|delete/i',
        ] as $label => $pattern) {
        ...
        foreach ($keywords as $keyword) {
            ...
            if ($keyword === '' || mb_strlen($keyword) < 6 || preg_match('/\b(breach|contract|dispute|applicant|respondent)\b/iu', $keyword)) {
                continue;
            }
            ...
            $issues[] = mb_convert_case($keyword, MB_CASE_TITLE, 'UTF-8');
        }
    ...
    {
        if ($this->isLegislativeDocumentType((string) ($result['document_type'] ?? ''))) {
            return $this->legislativeSummaryQualityIssues($citizenSummary) !== [];
        }
    ...
    private function legislativeSummaryQualityIssues(string $summary): array
    {
        $issues = [];
        $lower = mb_strtolower($summary);

        foreach ([
            'uses court-case framing' => '/\b(main facts|court reasoned|court found|in everyday terms, the result|what happened|final ruling|facts are that)\b/u',
            'uses dispute-party language' => '/\b(applicant|respondent|plaintiff|defendant|accused|deceased)\b/u',
            'misclassifies legislative subject as contract breach' => '/\bbreach of contract\b/u',
        ] as $label => $pattern) {
        ...
        if (!preg_match('/\b(act|bill|statutory instrument|law|legislation|rules|provisions|requires|allows|regulates|duties|powers|offence|penalty|registration|register)\b/u', $lower)) {
            $issues[] = 'does not explain legislative rules or legal effect';
        }
