...
class NlpService
{
    ...
    public function __construct(
        private readonly NlpAdaptiveLearningService $adaptiveLearning
    ) {}
    ...
    public function analyse(string $text, string $filename = ''): array
    {
        ...
        $keywords = $this->tfidfKeywords($clean, $tokens);
    ...
    {
        $haystack = mb_strtolower($text . ' ' . $filename);
        $patterns = [
            'Bill' => [
                '/\b(this bill seeks to|amend(?:ment)?|insert(?:ion)?|repeal|minister may|be it enacted|amendment of section|insertion of new section|this act may be cited as|bill)\b/u' => 2.3,
                '/\b(constitution(?: of zimbabwe)? amendment|amendment bill|short title)\b/u' => 2.8,
            ],
            'Court Judgment' => [
                '/\b(held that|applicant|respondent|plaintiff|defendant|court finds|it is ordered|judgment|judgement|appeal dismissed|appeal|appellant|ruling)\b/u' => 1.6,
                '/\b(?:hch|hcc|hh|hc|sc|ccz?)\s*\d{1,6}(?:\/\d{2,4}|-\d{2,4})?\b/u' => 2.6,
                '/\b(high court|supreme court|constitutional court|labour court|magistrates court|administrative court|in the high court|before:)\b/u' => 2.4,
            ],
            'Act' => [
                '/\b(any person who|shall be guilty|penalt(?:y|ies)|regulations|subject to this act|offence|offense|statutory instrument|act no\.?|chapter\s+\d|first schedule)\b/u' => 2.1,
                '/\b(shall be liable|convicted|fined|imprisonment)\b/u' => 1.8,
            ],
            'Lease Agreement' => [
                '/\b(tenant|landlord|premises|rent|lease term|security deposit|monthly rental|deposit|lessee|lessor)\b/u' => 2.0,
                '/\b(lease agreement|rental|occupancy|property let)\b/u' => 1.8,
            ],
            'Contract Agreement' => [
                '/\b(the parties agree|confidentiality|breach|indemnity|liability|termination|agreement|contract|whereas|consideration|parties agree)\b/u' => 1.6,
                '/\b(employment|employer|employee|salary|notice period|probation|leave entitlement|purchase price|vendor|purchaser|loan|borrower|lender|repayment|shareholder|board of directors)\b/u' => 1.3,
            ],
        ];
        ...
        if (($scores['Lease Agreement'] ?? 0) >= (($scores['Contract Agreement'] ?? 0) + 0.8)) {
            return 'Lease Agreement';
        }

        if (($scores['Bill'] ?? 0) >= 2.4 && ($scores['Bill'] ?? 0) >= (($scores['Act'] ?? 0) + 0.5)) {
            return 'Bill';
        }

        $bestLabel = 'Contract Agreement';
        $bestScore = 0.0;
        ...
        return $bestScore >= 1.2 ? $bestLabel : 'Contract Agreement';
    }
    ...
    private function extractEntities(string $text): array
    {
        ...
        preg_match_all('/\b(USD?\s*[\d,]+(?:\.\d{2})?|ZWL?\s*[\d,]+(?:\.\d{2})?|\$[\d,]+(?:\.\d{2})?)\b/i', $text, $amounts);
        $entities['amounts'] = array_values(array_unique($amounts[1] ?? []));
    ...
    private function tfidfKeywords(string $text, array $tokens): array
    {
        ...
        foreach ($this->adaptiveLearning->keywordBoosts($text) as $term => $boost) {
    ...
    {
        return preg_match('/^(?:page\s+\d+|\d+|harare|bulawayo|mutare)$/iu', $line) === 1;
    }
    ...
    private function detectOutcome(string $text, array $sentences, string $docType): string
    {
        ...
        $patterns = [
            '/(?:it\s+is\s+ordered|it\s+is\s+hereby\s+ordered|the\s+application\s+is\s+(?:granted|dismissed)|the\s+appeal\s+(?:succeeds|fails)|declared?\s+null(?:ity)?|null\s+and\s+void|certificate(?:s)?\s+(?:is|are)\s+cancelled|with\s+costs(?:\s+on\s+a\s+higher\s+scale)?)[^.]{0,260}\./iu',
    ...
    {
        if (preg_match('/\b(environmental impact assessment|eiac|environmental management agency|mining)\b/i', $text)) {
    ...
    private function legalCategories(string $text): array
    {
        ...
        return array_values(array_unique(array_merge($categories, $this->adaptiveLearning->categoriesForText($text))));
    }
    ...
    private function buildExecutiveSummary(
        ...
        if ($this->isBillDocument($docType)) {
            $summary = $this->buildExpandedLegislativeSummary($text, $metaFields, $topSentences);
            if ($summary !== '') {
                return $summary;
            }
        }
    ...
    private function buildKeyFindings(string $text, string $docType, array $findings): string
    {
        if ($this->isBillDocument($docType)) {
            ...
            if (count($highlights) < 3) {
                $highlights = array_values(array_unique(array_merge($highlights, $this->extractLegislativeHighlights($this->sentences($text), 3))));
                $highlights = array_slice($highlights, 0, 3);
            }
            if (!empty($highlights)) {
                return $this->joinHighlights($highlights);
            }
        }
    ...
    private function buildSummaryIntro(string $docType, array $metaFields, array $entities): string
    {
        ...
        if ($this->isBillDocument($docType)) {
            $title = $this->extractBillTitle($this->flattenSummaryText([$docType, $date]));
            $sentence = 'This bill outlines proposed changes to the law in plain terms.';
            if ($date) {
                $sentence .= ' The draft text is dated ' . $date . '.';
            }
    ...
    private function buildExpandedLegislativeSummary(string $text, array $metaFields, array $topSentences): string
    {
        $highlights = $this->extractLegislativeHighlightsFromText($text, 3);
        if (count($highlights) < 3) {
            $highlights = array_values(array_unique(array_merge($highlights, $this->extractLegislativeHighlights($this->sentences($text), 3))));
        ...
        $title = $this->extractBillTitle($text);
        $sentences = [];

        if ($title) {
            $sentences[] = 'This bill, ' . preg_replace('/\s+/u', ' ', trim($title)) . ', explains proposed legal changes in plain language.';
        } else {
            $sentences[] = 'This bill explains proposed legal changes in plain language.';
        }

        if (!empty($metaFields['date'])) {
            $sentences[] = 'The extracted draft text is dated ' . $metaFields['date'] . '.';
        }

        foreach ($highlights as $highlight) {
            $plain = $this->rewriteSentenceInPlainEnglish($highlight);
            if ($plain !== '' && !$this->isRedundantSentence($plain, $sentences)) {
                $sentences[] = 'In simple terms, it ' . preg_replace('/^[A-Z]/u', strtolower(mb_substr($plain, 0, 1)) . mb_substr($plain, 1), rtrim($plain, '.')) . '.';
            }
        }

        foreach ($this->buildSummaryFactSentences($topSentences, 2) as $factSentence) {
            if (!$this->isRedundantSentence($factSentence, $sentences)) {
                $sentences[] = $factSentence;
            }
        }

        $sentences[] = 'For a non-expert reader, the key point is that this is still a proposal rather than a final law, so the practical effect depends on whether Parliament passes it and the final wording remains the same.';
    ...
    private function isInvalidJudgeCandidate(string $candidate): bool
    {
        ...
        return (bool) preg_match('/\b(court|zimbabwe|held|harare|bulawayo|mutare|gweru|masvingo|case|number|applicant|respondent|plaintiff|defendant|judgment|judgement|justice|judge|honourable)\b/iu', $candidate);
    }
    ...
    {
        return in_array($docType, ['Bill', 'Act'], true);
    }
