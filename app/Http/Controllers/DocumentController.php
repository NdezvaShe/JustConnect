...
class DocumentController extends Controller
{
    ...
    {
        $request->validate([
            'file' => 'required|file|mimes:pdf,txt,doc,docx|max:20480', // 20 MB
        ]);
    ...
    public function extract(Request $request, DocumentTextExtractionService $extractor)
    {
        $request->validate([
            'file' => 'required|file|mimes:txt,doc,docx|max:20480',
        ]);

        $payload = $extractor->extract($request->file('file'));
        if (mb_strlen(trim((string) $payload['text'])) < 20) {
